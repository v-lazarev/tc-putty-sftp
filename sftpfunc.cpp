...
BOOL ReadChannelLine(LIBSSH2_CHANNEL *channel, char *line, int linelen, char *msgbuf, int msgbuflen, char *errbuf,
                     int errbuflen);
int CloseRemote(void *serverid, LIBSSH2_SFTP_HANDLE *remotefilesftp, LIBSSH2_CHANNEL *remotefilescp, BOOL timeout,
                int percent);
...
void OverwriteWithZeroes(char *buf, int len);
void OverwriteStackWithZeroes();
static void LoadSshConfigSettings(const char *HostName, pConnectSettings ConnectResults);

struct TemporarySshConfigOverrideGuard
{
    ...
    TemporarySshConfigOverrideGuard(pConnectSettings connectSettings)
        : ConnectSettings(connectSettings), customport(connectSettings->customport)
    {
        strlcpy(server, connectSettings->server, sizeof(server) - 1);
        strlcpy(user, connectSettings->user, sizeof(user) - 1);
    ...
    ~TemporarySshConfigOverrideGuard()
    {
        strlcpy(ConnectSettings->server, server, sizeof(ConnectSettings->server) - 1);
        strlcpy(ConnectSettings->user, user, sizeof(ConnectSettings->user) - 1);
        strlcpy(ConnectSettings->pubkeyfile, pubkeyfile, sizeof(ConnectSettings->pubkeyfile) - 1);
        strlcpy(ConnectSettings->privkeyfile, privkeyfile, sizeof(ConnectSettings->privkeyfile) - 1);
        ConnectSettings->customport = customport;
    }
};

typedef ADDRINFOA ADDRINFO, FAR *LPADDRINFO;

typedef int(WINAPI *tgetaddrinfo)(IN const char FAR *nodename, IN const char FAR *servname,
...
void SplitString(const char *szIn, char cDelim, std::vector<DWORD> *pv)
{
    if (szIn == NULL || pv == NULL)
        return;

    pv->clear();

    char *szFP = (char *)strchr(szIn, cDelim);
    char *szOld = (char *)szIn;

    if (szFP != NULL)
    {
        while (szFP != NULL)
        {
            pv->push_back(MAKELONG((WORD)(szOld - szIn), (WORD)(szFP - szOld))); // add pair with pos and length

            szOld = szFP + 1;

            szFP = strchr(szOld, cDelim);

            if (szFP == NULL && strlen(szOld) > 0)
                pv->push_back(MAKELONG((WORD)(szOld - szIn),
                                       (WORD)(&szIn[strlen(szIn)] - szOld))); // add pair with pos and length
        }
    }
}

// not needed anymore. It is now defined in libssh2.h
// typedef void* _LIBSSH2_AGENT;
...
BOOL LoadSSHLib()
{
    if (!sshlib)
    {
        ...
        {
            p[0] = 0;
            strlcat(dllname, "64\\libcrypto-1_1-x64.dll", sizeof(dllname) - 1);
            sshlib = (HINSTANCE)LoadLibrary(dllname);
            p[0] = 0;
            strlcat(dllname, "64\\libssl-1_1-x64.dll", sizeof(dllname) - 1);
            sshlib = (HINSTANCE)LoadLibrary(dllname);
            p[0] = 0;
            strlcat(dllname, "64\\libssh2.dll", sizeof(dllname) - 1);
            sshlib = (HINSTANCE)LoadLibrary(dllname);
        ...
        if (!sshlib)
        {
                ...
                if (sshlib)
                {
                    ...
                    if (icon)
                    {
                        MessageBox(GetActiveWindow(),
                                   "This plugin requires Windows Vista, 7 or newer. Please get the separate plugin for "
                                   "Windows XP or older from www.ghisler.com!",
                                   "Error", MB_ICONSTOP);
                        return false;
                    }
                }
            ...
            int res = MessageBox(GetActiveWindow(),
                                 "Please put the openssl dlls either\n- in the same directory as the plugin, or\n- in "
                                 "the Total Commander dir, or\n- in subdir \"64\" of the plugin or TC directory, or\n- "
                                 "somewhere in your PATH!\n\nDownload now?",
                                 "Error", MB_YESNO | MB_ICONSTOP);
#else
            int res = MessageBox(GetActiveWindow(),
                                 "Please put the openssl dlls either\n- in the same directory as the plugin, or\n- in "
                                 "the Total Commander dir, or\n- somewhere in your PATH!\n\nDownload now?",
                                 "Error", MB_YESNO | MB_ICONQUESTION);
#endif
            if (res == IDYES)
...
#include "sshdynfunctions.h"
        ...
        SSH_ScpNo2GBLimit =
            (libssh2_version != NULL && libssh2_version(LIBSSH2_VERSION_NUM_64BIT_FILETRANSFER_SCP) != NULL);
        SSH_ScpCanSendKeepAlive =
            (libssh2_version != NULL && libssh2_version(LIBSSH2_VERSION_NUM_CAN_SEND_KEEP_ALIVE_SCP) != NULL);
...
static void DecodeProtectedPassword(char *password, pProtectedPassword pwd)
{
    DecryptString(pwd->password, password, MAX_PASSWORD);
}
    ...
    for (int i = 0; i < num_prompts; i++)
    {
        ...
        if (!autoSendPassword)
        {
            ...
            if (RequestProc(PluginNumber, RT_Password, title, buf, retbuf, sizeof(retbuf) - 1))
            {
                ...
                if (ConnectSettings && ConnectSettings->protectedpassword.length == 0)
                    EncodeProtectedPassword(&ConnectSettings->protectedpassword, retbuf);
            ...
            OverwriteWithZeroes(retbuf, sizeof(retbuf));
        }
    }
} /* kbd_callback */
...
void SftpLogMethod(const char *methodname, const char *method)
{
    if (!method)
        return;
    char errbuf[256];
    strlcpy(errbuf, methodname, sizeof(errbuf) - 1);
    strlcat(errbuf, ": ", sizeof(errbuf) - 1);
    strlcat(errbuf, method, sizeof(errbuf) - 1);
    LogProc(PluginNumber, MSGTYPE_IMPORTANTERROR, errbuf);
}
...
int mysend(SOCKET s, const char *buf, int len, int flags, char *progressmessage, int progressstart, int *ploop,
           DWORD *plasttime)
{
    int err;
...
#ifdef _DEBUG
        // should not happen but in case, the user doesn't know where this beep is comming from...
        if (ret != len)
            MessageBeep(0);
#endif
        if (ret >= 0)
...
int myrecv(SOCKET s, char *buf, int len, int flags, char *progressmessage, int progressstart, int *ploop,
           DWORD *plasttime)
{
    int err;
    int totallen = len;
...
void newpassfunc(LIBSSH2_SESSION *session, char **newpw, int *newpw_len, void **abstract)
{
    ...
    if (RequestProc(PluginNumber, RT_Password, title, buf1, newpass, sizeof(newpass) - 1))
    {
        ...
        *newpw_len = bufsize;
        ...
        {
            EncodeProtectedPassword(&PassConnectSettings->protectedpassword, newpass);
            switch (PassConnectSettings->passSaveMode)
            {
            case 1:
                CryptProc(PluginNumber, CryptoNumber, FS_CRYPT_SAVE_PASSWORD, PassConnectSettings->DisplayName, newpass,
                          0);
                break;
            case 2:
                if (newpass[0] == 0)
                ...
                else
                {
                    ...
                    WritePrivateProfileString(PassConnectSettings->DisplayName, "password", szEncryptedPassword,
                                              gIniFileName);
                }
                break;
            }
    ...
    OverwriteWithZeroes(newpass, sizeof(newpass));
}

void libssh2_trace_handler_callback(LIBSSH2_SESSION *session, void *param, const char *error, size_t len)
{
...
int SftpConnect(pConnectSettings ConnectSettings)
{
    ...
    {
        const char *sshConfigName =
            ConnectSettings->server[0] != 0 ? ConnectSettings->server : ConnectSettings->DisplayName;
        if (sshConfigName[0] != 0)
        {
    ...
    if (!ConnectSettings->session)
    {
        ...
        {
            LogConnectionAttempt(ConnectSettings->DisplayName, ConnectSettings->server, ConnectSettings->customport,
                                 ConnectSettings->user, ConnectSettings->proxytype, "FAILED",
                                 ConnectSettings->proxytype ? "Proxy connection failed" : "Server connection failed");
            if (ConnectSettings->proxytype)
        ...
        case 2: // HTTP CONNECT
            ...
            if (nrbytes == 12 && buf[9] == '2')
            { // proxy signals success!!
        ...
        case 3: // SOCKS4/4A
            ...
            *((unsigned short *)&buf[2]) = htons(ConnectSettings->customport);
            ...
            if (nrbytes != 8 || buf[0] != 0 || buf[1] != 0x5a)
            {
                LogConnectionAttempt(ConnectSettings->DisplayName, ConnectSettings->server, ConnectSettings->customport,
                                     ConnectSettings->user, ConnectSettings->proxytype, "FAILED",
                                     "SOCKS4/4a proxy connect failed");
                ShowErrorId(IDS_VIA_PROXY_CONNECT);
                closesocket(ConnectSettings->sock);
                ConnectSettings->sock = 0;
                return -1;
        ...
        case 4: // SOCKS5
            ...
            else
            {
                ...
                if (pgetaddrinfo && IsNumericIPv6(ConnectSettings->server))
                {
                    memset(&hints, 0, sizeof(hints));
                    hints.ai_family = AF_INET6;
                    hints.ai_socktype = SOCK_STREAM;
            ...
            *((unsigned short *)&buf[nrbytes]) = htons(ConnectSettings->customport);
            ...
            if (nrbytes != 4 || buf[0] != 5 || buf[1] != 0)
            {
                ...
                {
                case 1:
                    LoadStr(buf, IDS_GENERALSOCKSFAILURE);
                    break;
                case 2:
                    LoadStr(buf, IDS_CONNNOTALLOWED);
                    break;
                case 3:
                    LoadStr(buf, IDS_NETUNREACHABLE);
                    break;
                case 4:
                    LoadStr(buf, IDS_HOSTUNREACHABLE);
                    break;
                case 5:
                    LoadStr(buf, IDS_CONNREFUSED);
                    break;
                case 6:
                    LoadStr(buf, IDS_TTLEXPIRED);
                    break;
                case 7:
                    LoadStr(buf, IDS_CMDNOTSUPPORTED);
                    break;
                case 8:
                    LoadStr(buf, IDS_ADDRTYPENOTSUPPORTED);
                    break;
                default: {
                    char buf2[MAX_PATH];
        ...
        libssh2_trace_sethandler(ConnectSettings->session, NULL, libssh2_trace_handler_callback);
        libssh2_trace(ConnectSettings->session, ConnectSettings->debuglog);
        ...
        while ((err = libssh2_session_method_pref(ConnectSettings->session, LIBSSH2_METHOD_COMP_CS,
                                                  ConnectSettings->compressed ? "zlib,none" : "none")) ==
        ...
        while ((err = libssh2_session_method_pref(ConnectSettings->session, LIBSSH2_METHOD_COMP_SC,
                                                  ConnectSettings->compressed ? "zlib,none" : "none")) ==
               LIBSSH2_ERROR_EAGAIN)
        {
            if (ProgressLoop(buf, 30, 40, &loop, &lasttime))
        ...
        for (int method = 0; method <= 9; method++)
        {
            ...
            if (prefs[0])
            {
                while ((err = libssh2_session_method_pref(ConnectSettings->session, method, prefs)) ==
                       LIBSSH2_ERROR_EAGAIN)
                {
                    if (ProgressLoop(buf, 30, 40, &loop, &lasttime))
                        break;
        ...
        while ((auth = libssh2_session_startup(ConnectSettings->session, (int)ConnectSettings->sock)) ==
               LIBSSH2_ERROR_EAGAIN)
        {
        ...
        // log methods:
        SftpLogMethod("Key exchange method", libssh2_session_methods(ConnectSettings->session, LIBSSH2_METHOD_KEX));
        SftpLogMethod("Host key method", libssh2_session_methods(ConnectSettings->session, LIBSSH2_METHOD_HOSTKEY));
        const char *cryptcs = libssh2_session_methods(ConnectSettings->session, LIBSSH2_METHOD_CRYPT_CS);
        const char *cryptsc = libssh2_session_methods(ConnectSettings->session, LIBSSH2_METHOD_CRYPT_SC);
        if (cryptcs != NULL && cryptsc != NULL && strcmp(cryptcs, cryptsc) == 0)
            SftpLogMethod("Encryption method (both directions)", cryptcs);
        else
        {
            SftpLogMethod("Encryption method (client to server)", cryptcs);
            SftpLogMethod("Encryption method (server to client)", cryptsc);
        }
        cryptcs = libssh2_session_methods(ConnectSettings->session, LIBSSH2_METHOD_MAC_CS);
        cryptsc = libssh2_session_methods(ConnectSettings->session, LIBSSH2_METHOD_MAC_SC);
        if (cryptcs != NULL && cryptsc != NULL && strcmp(cryptcs, cryptsc) == 0)
            SftpLogMethod("MAC (message authentication code) method (both directions)", cryptcs);
        else
        {
            SftpLogMethod("MAC (message authentication code) method (client to server)", cryptcs);
            SftpLogMethod("MAC (message authentication code) method (server to client)", cryptsc);
        }
        cryptcs = libssh2_session_methods(ConnectSettings->session, LIBSSH2_METHOD_COMP_CS);
        cryptsc = libssh2_session_methods(ConnectSettings->session, LIBSSH2_METHOD_COMP_SC);
        if (cryptcs != NULL && cryptsc != NULL && strcmp(cryptcs, cryptsc) == 0)
        {
            if (strcmp(cryptcs, "none") != 0)
                SftpLogMethod("Compression method (both directions)", cryptcs);
        }
        else
        {
            if (cryptcs != NULL && strcmp(cryptcs, "none") != 0)
                SftpLogMethod("Compression method (client to server)", cryptcs);
            if (cryptsc != NULL && strcmp(cryptsc, "none") != 0)
                SftpLogMethod("Compression method (server to client)", cryptsc);
        }
        if (auth)
        {
            ...
            LogConnectionAttempt(ConnectSettings->DisplayName, ConnectSettings->server, ConnectSettings->customport,
                                 ConnectSettings->user, ConnectSettings->proxytype, "FAILED", errmsg);
            ShowError(buf);
        ...
        else
        {
            ...
            { // a new server, or changed fingerprint
                ...
                WritePrivateProfileString(ConnectSettings->DisplayName, "fingerprint", buf,
                                          ConnectSettings->IniFileName);
        ...
        if (ConnectSettings->user[0] == 0)
        {
            ...
            strlcat(title, ConnectSettings->server, sizeof(title) - 1);
            if (!RequestProc(PluginNumber, RT_UserName, title, NULL, ConnectSettings->user,
                             sizeof(ConnectSettings->user) - 1))
            {
                libssh2_session_free(ConnectSettings->session);
                ConnectSettings->session = NULL;
                Sleep(1000);
        ...
        do
        {
            userauthlist = libssh2_userauth_list(ConnectSettings->session, ConnectSettings->user,
                                                 (unsigned int)strlen(ConnectSettings->user));
        ...
        else if (auth_pw & 4 && ConnectSettings->useagent && loadAgent)
        {
            ...
            if (!agent || libssh2_agent_connect(agent) != 0)
            {
                ...
                if (GetFileAttributes(linkname) != 0xFFFFFFFF)
                {
                    ...
                    while (active != GetForegroundWindow() && abs((int)(GetCurrentTime() - starttime)) < 20000)
                    {
                        Sleep(200);
            ...
            if (connected)
            {
                ...
                else
                {
                    while (1)
                    {
                        ...
                        strlcat(buf, buf1, sizeof(buf) - 1);
                        ShowStatus(buf);
                        while ((auth = libssh2_agent_userauth(agent, ConnectSettings->user, identity)) ==
                               LIBSSH2_ERROR_EAGAIN)
                            ;
                        if (auth)
                        {
        ...
        else if (auth_pw & 4 && ConnectSettings->privkeyfile[0])
        {
            BOOL pubkeybad = false;
            BOOL haspubkey = ConnectSettings->pubkeyfile[0] != 0;
            char filebuf[1024];
            char passphrase[256];
            ...
            if (haspubkey)
            {
                ...
                HANDLE hf = CreateFile(pubkeyfile, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL,
                ...
                else
                {
                    ...
                    {
                        if (_strnicmp(filebuf, "ssh-", 4) != 0 && _strnicmp(filebuf, "ecdsa-", 6) != 0 &&
            ...
            if (!pubkeybad)
            {
                ...
                HANDLE hf = CreateFile(privkeyfile, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL,
                                       OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_SEQUENTIAL_SCAN, NULL);
                if (hf == INVALID_HANDLE_VALUE)
                {
                ...
                else
                {
                    ...
                    if (isencrypted)
                    {
                        ...
                        if (ConnectSettings->protectedpassword.length > 0)
                        {
                            ...
                            if (p && password[0] == '"' && password[len - 1] == '"')
                            {
                                // two passwords -> use second one!
                                p[0] = 0;
                                strlcpy(passphrase, password + 1, sizeof(passphrase) - 1);
                                p[0] = '"';
                            }
                            else
                                strlcpy(passphrase, password, sizeof(passphrase) - 1);
                            OverwriteWithZeroes(password, sizeof(password));
                        }
                        else
                    ...
                    else if (ConnectSettings->protectedpassword.length == 0)
                        EncodeProtectedPassword(&ConnectSettings->protectedpassword, passphrase);
        ...
        if ((auth_pw & 4) == 0)
        {
            ...
            { // keyboard-interactive
                ...
                while ((auth = libssh2_userauth_keyboard_interactive(ConnectSettings->session, ConnectSettings->user,
        ...
        OverwriteStackWithZeroes();

        if (auth)
        {
            LogConnectionAttempt(ConnectSettings->DisplayName, ConnectSettings->server, ConnectSettings->customport,
                                 ConnectSettings->user, ConnectSettings->proxytype, "FAILED", "Authentication failed");
        ...
        if (ConnectSettings->utf8names == -1)
        {
            ...
            if (strcmp(ConnectSettings->DisplayName, s_quickconnect) != 0)
                WritePrivateProfileString(ConnectSettings->DisplayName, "utf8", ConnectSettings->utf8names ? "1" : "0",
                                          ConnectSettings->IniFileName);
        }
        if (ConnectSettings->unixlinebreaks == -1)
        {
            ...
            // store the result!
            if (strcmp(ConnectSettings->DisplayName, s_quickconnect) != 0)
                WritePrivateProfileString(ConnectSettings->DisplayName, "unixlinebreaks",
                                          ConnectSettings->unixlinebreaks ? "1" : "0", ConnectSettings->IniFileName);
        }
        ...
        if (ConnectSettings->scpfordata && ConnectSettings->scpserver64bit == -1)
        {
            ...
            // store the result!
            if (strcmp(ConnectSettings->DisplayName, s_quickconnect) != 0)
                WritePrivateProfileString(ConnectSettings->DisplayName, "largefilesupport",
                                          ConnectSettings->scpserver64bit ? "1" : "0", ConnectSettings->IniFileName);
        }
        ...
        if (!ConnectSettings->scponly)
        {
            ...
            do
            {
                ...
                if ((!ConnectSettings->sftpsession) &&
                    (libssh2_session_last_errno(ConnectSettings->session) != LIBSSH2_ERROR_EAGAIN))
                {
                    break;
                }
                IsSocketReadable(ConnectSettings->sock); // sleep to avoid 100% CPU!
    ...
    if (ConnectSettings->scponly)
    {
        ...
        {
            LogConnectionAttempt(ConnectSettings->DisplayName, ConnectSettings->server, ConnectSettings->customport,
                                 ConnectSettings->user, ConnectSettings->proxytype, "SUCCESS",
                                 "SCP-only connection established");
            return SFTP_OK;
        }
    }
...
void EncryptString(LPCTSTR pszPlain, LPTSTR pszEncrypted, UINT cchEncrypted)
{
    ...
    pszEncrypted[0] = '\0';
    if (3 * iPlainLength >= (int)cchEncrypted)
        iPlainLength = cchEncrypted / 3 - 1;
...
void DecryptString(LPCTSTR pszEncrypted, LPTSTR pszPlain, UINT cchPlain)
{
    if (strcmp(pszEncrypted, "!") == 0)
    { // signal password-protected password
        if (CryptProc)
...
BOOL LoadProxySettingsFromNr(int proxynr, pConnectSettings ConnectResults)
{
    if (proxynr > 0)
    {
        ...
        GetPrivateProfileString(proxyentry, "proxyserver", "", ConnectResults->proxyserver,
                                sizeof(ConnectResults->proxyuser) - 1, gIniFileName);
        GetPrivateProfileString(proxyentry, "proxyuser", "", ConnectResults->proxyuser,
                                sizeof(ConnectResults->proxyuser) - 1, gIniFileName);
...
// Values collected from a single matching "Host" block of the SSH config file.
struct SshConfigMatch
{
    char hostName[MAX_PATH];
    char user[MAX_PATH];
    char identityFile[MAX_PATH];
    int port;
    bool hasHostName;
...
// Expand a leading "~" to the user profile folder and normalise '/' to '\\'.
static void ExpandSshConfigPath(const char *value, const char *profilePath, char *out, size_t outSize)
{
    if (value[0] == '~')
    {
...
// Copy the values resolved from a matching Host block into the active connection
...
static void ApplySshConfigMatch(const SshConfigMatch &match, const char *requestedHost, pConnectSettings ConnectResults)
{
    ...
    if (ConnectResults->user[0] == 0 && match.user[0] != 0)
        strlcpy(ConnectResults->user, match.user, sizeof(ConnectResults->user) - 1);

    // Identity (private key) file, plus its matching .pub file if one exists.
    if (ConnectResults->privkeyfile[0] == 0 && match.identityFile[0] != 0 && !ConnectResults->useagent)
    {
        strlcpy(ConnectResults->privkeyfile, match.identityFile, sizeof(ConnectResults->privkeyfile) - 1);
        if (ConnectResults->pubkeyfile[0] == 0)
        {
            ...
            if (GetFileAttributesA(pubKeyFile) != INVALID_FILE_ATTRIBUTES)
                ...
                strlcpy(ConnectResults->pubkeyfile, match.identityFile, sizeof(ConnectResults->pubkeyfile) - 1);
        }
    }
}

// Read %USERPROFILE%\.ssh\config and apply the first Host block matching HostName.
static void LoadSshConfigSettings(const char *HostName, pConnectSettings ConnectResults)
{
    char profilePath[MAX_PATH];
    if (!GetEnvironmentVariableA("USERPROFILE", profilePath, sizeof(profilePath)))
        return;
    ...
    SshConfigMatch match;
    match.Reset();
    bool inMatchingHost = false;
    ...
    while (fgets(line, sizeof(line), f))
    {
        ...
        // Skip leading whitespace, then ignore blank lines and comments.
        char *key = line;
        while (*key == ' ' || *key == '\t')
            key++;
        if (*key == '#' || *key == 0)
        ...
        // Split into "key value"; the value may be separated by space, tab or '='.
        char *val = key;
        ...
        {
            // A new Host block starts: flush the previous one first.
            if (inMatchingHost)
                ApplySshConfigMatch(match, HostName, ConnectResults);

            match.Reset();
            inMatchingHost = false;

            // This block applies if any of its patterns matches HostName.
            char *next = NULL;
            for (char *tok = strtok_s(val, " \t", &next); tok; tok = strtok_s(NULL, " \t", &next))
            {
                if (MatchPattern(tok, HostName))
                {
                    inMatchingHost = true;
                    break;
                }
        ...
        else if (inMatchingHost)
        {
            ...
            {
                strlcpy(match.hostName, val, sizeof(match.hostName) - 1);
                match.hasHostName = true;
            }
            else if (_stricmp(key, "User") == 0)
            {
            ...
            {
                ExpandSshConfigPath(val, profilePath, match.identityFile, sizeof(match.identityFile));
            }
        }
    }
...
{
    char szPassword[MAX_PASSWORD], modbuf[6];
    ...
    GetPrivateProfileString(DisplayName, "fingerprint", "", ConnectResults->savedfingerprint,
                            sizeof(ConnectResults->savedfingerprint) - 1, gIniFileName);
    GetPrivateProfileString(DisplayName, "pubkeyfile", "", ConnectResults->pubkeyfile,
                            sizeof(ConnectResults->pubkeyfile) - 1, gIniFileName);
    GetPrivateProfileString(DisplayName, "privkeyfile", "", ConnectResults->privkeyfile,
                            sizeof(ConnectResults->privkeyfile) - 1, gIniFileName);
    ...
    ConnectResults->debuglog = GetPrivateProfileInt(gDisplayName, "debuglog", 128 /* errors only */, gIniFileName);
    ...
    ConnectResults->unixlinebreaks =
        GetPrivateProfileInt(gDisplayName, "unixlinebreaks", -1, gIniFileName); // -1 means auto-detect
    ConnectResults->scpserver64bit =
        GetPrivateProfileInt(gDisplayName, "largefilesupport", -1, gIniFileName); // -1 means auto-detect
    ConnectResults->protectedpassword.length = 0;
    ...
    if (GetPrivateProfileString(gDisplayName, "password", "", szPassword, countof(szPassword), gIniFileName))
    {
        if (strcmp(szPassword, "!") == 0)
            ConnectResults->protectedpassword.length = -1; // signals password stored with master password
        else if (!ConnectResults->useagent)
        {
            char password[MAX_PASSWORD];
            DecryptString(szPassword, password, countof(password));
            EncodeProtectedPassword(&ConnectResults->protectedpassword, password);
            OverwriteWithZeroes(password, sizeof(password));
        }
    }
    ...
    GetPrivateProfileString(DisplayName, "sendcommand", "", ConnectResults->connectsendcommand,
                            sizeof(ConnectResults->connectsendcommand) - 1, gIniFileName);
...
int codepagelist[] = {-1,   -2,   0,   1,   2,   1250, 1251, 1252, 1253, 1254,  1255, 1256,
                      1257, 1258, 936, 950, 932, 949,  874,  437,  850,  20866, -3,   -4};
...
myint __stdcall ProxyDlgProc(HWND hWnd, unsigned int Message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG: {
        ...
        {
        case 2:
            g_focusset = IDC_OTHERPROXY;
            break;
        case 3:
            g_focusset = IDC_SOCKS4APROXY;
            break;
        case 4:
            g_focusset = IDC_SOCKS5PROXY;
            break;
        default:
        ...
        if (strcmp(ConnectData.proxypassword, "\001") == 0 && CryptProc)
        {
            ...
            if (CryptProc(PluginNumber, CryptoNumber, FS_CRYPT_LOAD_PASSWORD_NO_UI, proxyentry,
                          ConnectData.proxypassword, countof(ConnectData.proxypassword) - 1) == FS_FILE_OK)
            {
    ...
    case WM_COMMAND: {
        ...
        case IDOK: {
            ...
            { // button not visible
                ...
                else if (CryptProc && IsDlgButtonChecked(hWnd, IDC_CRYPTPASS))
                {
                    ...
                    BOOL ok = CryptProc(PluginNumber, CryptoNumber, FS_CRYPT_SAVE_PASSWORD, proxyentry2,
                                        ConnectData.proxypassword, 0) == FS_FILE_OK;
        ...
        case IDC_EDITPASS: {
            ...
            if (gProxyNr > 1)
                ...
                strlcpy(proxyentry, "proxy", sizeof(proxyentry) - 1);
            strlcat(proxyentry, "$$pass", sizeof(proxyentry) - 1);
            err = CryptProc(PluginNumber, CryptoNumber, FS_CRYPT_LOAD_PASSWORD, proxyentry, ConnectData.proxypassword,
                            countof(ConnectData.proxypassword) - 1);
            if (err == FS_FILE_OK)
            {
                SetDlgItemText(hWnd, IDC_PROXYPASSWORD, ConnectData.proxypassword);
            }
            ...
            if (doshow)
            {
                ...
                if (gConnectResults->protectedpassword.length > 0)
                    CheckDlgButton(hWnd, IDC_CRYPTPASS, BST_CHECKED);
            }
        }
        }
...
myint __stdcall ConnectDlgProc(HWND hWnd, unsigned int Message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG: {
        ...
        if (strcmp(gDisplayName, s_quickconnect) != 0)
        {
            ...
            {
            case 1:
                CheckRadioButton(hWnd, IDC_PROTOAUTO, IDC_PROTOSSHCONFIG, IDC_PROTOV4);
                break;
            case 2:
                CheckRadioButton(hWnd, IDC_PROTOAUTO, IDC_PROTOSSHCONFIG, IDC_PROTOV6);
                break;
            case 3:
                CheckRadioButton(hWnd, IDC_PROTOAUTO, IDC_PROTOSSHCONFIG, IDC_PROTOSSHCONFIG);
                break;
            default:
                CheckRadioButton(hWnd, IDC_PROTOAUTO, IDC_PROTOSSHCONFIG, IDC_PROTOAUTO);
                break;
            }
            ...
            if (gConnectResults->protectedpassword.length == -1 && CryptProc)
            {
                password[0] = 0;
                if (/*!gConnectResults->useagent &&*/ CryptProc(PluginNumber, CryptoNumber,
                                                                FS_CRYPT_LOAD_PASSWORD_NO_UI, gDisplayName, password,
                                                                countof(password) - 1) == FS_FILE_OK)
                {
                    SetDlgItemText(hWnd, IDC_PASSWORD, password);
                    EncodeProtectedPassword(&gConnectResults->protectedpassword, password);
                    CheckDlgButton(hWnd, IDC_CRYPTPASS, BST_CHECKED);
                }
                else
                {
            ...
            {
                DecodeProtectedPassword(password, &gConnectResults->protectedpassword);
                SetDlgItemText(hWnd, IDC_PASSWORD, password);
                if (!CryptProc)
                    EnableWindow(GetDlgItem(hWnd, IDC_CRYPTPASS), false);
                else if (gConnectResults->protectedpassword.length == 0 && CryptCheckPass)
                    CheckDlgButton(hWnd, IDC_CRYPTPASS, BST_CHECKED);
            }
            OverwriteWithZeroes(password, sizeof(password));
    ...
    case WM_COMMAND: {
        ...
        case IDOK: {
            ...
            else if (IsDlgButtonChecked(hWnd, IDC_PROTOSSHCONFIG))
                gConnectResults->protocoltype = 3;
            else
            ...
            default:
                ...
                if (cbline >= 0 && cbline < countof(codepagelist))
                {
                    ...
                    if (cp == -3)
                    {
                        if (RequestProc(PluginNumber, RT_Other, "Code page", "Code page (e.g. 28591):", strbuf,
                                        sizeof(strbuf) - 1))
                        {
                            cp = atoi(strbuf);
                        }
            ...
            if (strcmp(gDisplayName, s_quickconnect) != 0)
            {
                ...
                WritePrivateProfileString(gDisplayName, "protocol", gConnectResults->protocoltype == 0 ? NULL : buf,
                                          gIniFileName);
                WritePrivateProfileString(gDisplayName, "detailedlog", gConnectResults->detailedlog ? "1" : NULL,
                                          gIniFileName);
                WritePrivateProfileString(gDisplayName, "utf8",
                                          gConnectResults->utf8names == -1  ? NULL
                                          : gConnectResults->utf8names == 1 ? "1"
                                                                            : "0",
                                          gIniFileName);
                _itoa_s(gConnectResults->codepage, buf, sizeof(buf), 10);
                WritePrivateProfileString(gDisplayName, "codepage", buf, gIniFileName);
                WritePrivateProfileString(gDisplayName, "unixlinebreaks",
                                          gConnectResults->unixlinebreaks == -1  ? NULL
                                          : gConnectResults->unixlinebreaks == 1 ? "1"
                                                                                 : "0",
                                          gIniFileName);
                WritePrivateProfileString(gDisplayName, "largefilesupport",
                                          gConnectResults->scpserver64bit == -1  ? NULL
                                          : gConnectResults->scpserver64bit == 1 ? "1"
                                                                                 : "0",
                                          gIniFileName);
                WritePrivateProfileString(gDisplayName, "compression", gConnectResults->compressed ? "1" : NULL,
                                          gIniFileName);
                WritePrivateProfileString(gDisplayName, "scpfordata", gConnectResults->scpfordata ? "1" : NULL,
                                          gIniFileName);
                _itoa_s(gConnectResults->keepAliveIntervalSeconds, buf, sizeof(buf), 10);
                WritePrivateProfileString(gDisplayName, "keepaliveseconds",
                                          gConnectResults->keepAliveIntervalSeconds == 0 ? NULL : buf, gIniFileName);
                WritePrivateProfileString(gDisplayName, "scponly", gConnectResults->scponly ? "1" : NULL, gIniFileName);
                WritePrivateProfileString(gDisplayName, "pubkeyfile",
                                          gConnectResults->pubkeyfile[0] ? gConnectResults->pubkeyfile : NULL,
                                          gIniFileName);
                WritePrivateProfileString(gDisplayName, "privkeyfile",
                                          gConnectResults->privkeyfile[0] ? gConnectResults->privkeyfile : NULL,
                                          gIniFileName);
                WritePrivateProfileString(gDisplayName, "useagent", gConnectResults->useagent ? "1" : NULL,
                                          gIniFileName);

                _itoa_s(gConnectResults->filemod, modbuf, sizeof(modbuf), 8);
                WritePrivateProfileString(gDisplayName, "filemod", gConnectResults->filemod == 0644 ? NULL : modbuf,
                                          gIniFileName);
                _itoa_s(gConnectResults->dirmod, modbuf, sizeof(modbuf), 8);
                WritePrivateProfileString(gDisplayName, "dirmod", gConnectResults->dirmod == 0755 ? NULL : modbuf,
                                          gIniFileName);
                ...
                {
                    TCHAR szEncryptedPassword[MAX_PASSWORD];
                    if (!IsWindowVisible(GetDlgItem(hWnd, IDC_EDITPASS)))
                    {
                        if (gConnectResults->protectedpassword.length == 0)
                        {
                        ...
                        {
                            DecodeProtectedPassword(password, &gConnectResults->protectedpassword);
                            BOOL ok = CryptProc(PluginNumber, CryptoNumber, FS_CRYPT_SAVE_PASSWORD, gDisplayName,
                                                password, 0) == FS_FILE_OK;
                        ...
                        {
                            DecodeProtectedPassword(password, &gConnectResults->protectedpassword);
                            EncryptString(password, szEncryptedPassword, countof(szEncryptedPassword));
        ...
        case IDC_EDITPASS: {
            ...
            int err;
            DecodeProtectedPassword(password, &gConnectResults->protectedpassword);
            err = CryptProc(PluginNumber, CryptoNumber, FS_CRYPT_LOAD_PASSWORD, gDisplayName, password,
                            countof(password) - 1);
            if (err == FS_FILE_OK)
            {
                EncodeProtectedPassword(&gConnectResults->protectedpassword, password);
...
BOOL ShowConnectDialog(pConnectSettings ConnectSettings, char *DisplayName, char *inifilename)
{
    ...
    {
        if ((ConnectSettings->user[0] == 0 || ConnectSettings->protectedpassword.length > 0) && // password saved
        ...
        else
        {
                ...
                ConnectSettings->proxypassword[0] == 0)
            {
                ...
                strlcat(title, ConnectSettings->proxyuser, sizeof(title) - 1);
                if (!RequestProc(PluginNumber, RT_PasswordFirewall, title, title, ConnectSettings->proxypassword,
                                 countof(ConnectSettings->proxypassword) - 1))
                    return false;
            }
            return true;
        }
...
void *SftpConnectToServer(char *DisplayName, char *inifilename, pProtectedPassword overridepass)
{
    ...
    ConnectSettings.wantKeepTimestamp = true;

    // Get connection settings here
    if (ShowConnectDialog(&ConnectSettings, DisplayName, inifilename))
    {
        if (overridepass)
            memcpy(&gConnectResults->protectedpassword, overridepass, sizeof(tProtectedPassword));
        if (CryptProc && (gConnectResults->protectedpassword.length == -1))
        {
            ConnectSettings.passSaveMode = 1;
            char password[MAX_PASSWORD];
            password[0] = 0;
            if (CryptProc(PluginNumber, CryptoNumber, FS_CRYPT_LOAD_PASSWORD, gDisplayName, password,
                          countof(password) - 1) != FS_FILE_OK)
            {
        ...
        else if (ConnectSettings.useagent || gConnectResults->protectedpassword.length == 0)
        ...
        if (CryptProc && strcmp(gConnectResults->proxypassword, "\001") == 0)
        {
            ...
            if (gConnectResults->proxynr > 1)
                ...
                strlcpy(proxyentry, "proxy", sizeof(proxyentry) - 1);
            strlcat(proxyentry, "$$pass", sizeof(proxyentry) - 1);
            if (CryptProc(PluginNumber, CryptoNumber, FS_CRYPT_LOAD_PASSWORD, proxyentry,
                          gConnectResults->proxypassword, countof(gConnectResults->proxypassword) - 1) != FS_FILE_OK)
            {
        ...
        else
        {
            ...
            {
                if (!(psettings->scpfordata && SSH_ScpNeedBlockingMode) && SSH_ScpCanSendKeepAlive &&
                    psettings->hWndKeepAlive == NULL)
                {
                    // only needed in non-blocking mode
                    psettings->hWndKeepAlive = ::CreateWindow("Static", "SFTPPlug keep alive window", WS_CHILD, 0, 0, 0,
                                                              0, HWND_MESSAGE, NULL, NULL, NULL);

                    if (psettings->hWndKeepAlive != NULL)
                    {
                        ghWndToConnectSettings[psettings->hWndKeepAlive] = psettings;
                        ::SetTimer(psettings->hWndKeepAlive, 1000, psettings->keepAliveIntervalSeconds * 1000,
                                   TimerProc);
                    }
                }
                else if (!SSH_ScpCanSendKeepAlive)
                {
                    strlcpy(connbuf, "KEEP-ALIVE not supported by libssh2. Version >= 1.2.5 required!",
                            sizeof(connbuf) - 1);
                    LogProc(PluginNumber, MSGTYPE_CONNECT, connbuf);
                }
...
int SftpFindFirstFileW(void *serverid, WCHAR *remotedir, void **davdataptr, char *skippedchar)
{
    ...
    WCHAR dirnamew[wdirtypemax];
    char dirname[wdirtypemax];
    char firstchar = 0;
    pConnectSettings ConnectSettings = (pConnectSettings)serverid;
    LoadStr(dirname, IDS_GET_DIR);
    MultiByteToWideChar(CP_ACP, 0, dirname, -1, (WCHAR *)&dirnamew, countof(dirnamew) - 1);
    wcslcat(dirnamew, remotedir, countof(dirnamew) - 1);

    ShowStatusW(dirnamew);
    ResetFileInfoExtras(ConnectSettings);
    for (int i = 0; i < 10; i++)
        if (EscapePressed())
    ...
    if (ConnectSettings->scponly)
    {
        ...
        for (int i = trycustom; i >= 0; i--)
        {
            ...
            if (rcerr == 0 || rcerr == LIBSSH2_ERROR_EAGAIN)
            { // custom failed, normal OK -> no longer try custom
                ConnectSettings->trycustomlistcommand = i;
                break;
            }
        }
        if (skippedchar)
            *skippedchar = firstchar;
            ...
            if (!dirhandle)
            {
                ...
                if (err != LIBSSH2_ERROR_EAGAIN)
                {
                    if (err == LIBSSH2_FX_EOF || err == LIBSSH2_FX_FAILURE || err == LIBSSH2_FX_BAD_MESSAGE ||
                        err == LIBSSH2_FX_NO_CONNECTION || err == LIBSSH2_FX_CONNECTION_LOST || err < 0)
                        retrycount--;
                    else
                        retrycount = 0;
                    if (retrycount <= 0)
...
BOOL SftpFindNextFileW(void *serverid, void *davdataptr, WIN32_FIND_DATAW *FindData, char skippedchar)
{
    ...
    char completeline[2048], *completelineptr;
    ...
    if (ConnectSettings->scponly)
    {
        ...
        if (!channel)
            return SFTP_FAILED;
        rc = 0;
        completelineptr = completeline;
        maxlen = sizeof(completeline) - 1;
        if (skippedchar)
        { // character received while requesting directory
            completeline[0] = skippedchar;
            completelineptr++;
            maxlen--;
        }
        while (ReadChannelLine(channel, completelineptr, maxlen, scpd->msgbuf, sizeof(scpd->msgbuf) - 1, scpd->errbuf,
                               sizeof(scpd->errbuf) - 1))
        {
            if (skippedchar)
            {
                completelineptr = completeline;
            ...
            if (ConnectSettings->utf8names)
            {
                ...
                ConvertUTF8toUTF16(&srcstart, srcstart + strlen(completeline) + 1, &trgstart,
                                   trgstart + countof(completelinew) - 1);
            }
            else
                awlcopyCP(ConnectSettings->codepage, completelinew, completeline, countof(completelinew) - 1);

            if (ReadDirLineUNIX(completelinew, namew, countof(namew) - 1, (__int64 *)&file.filesize, &datetime, &attr,
                                &file.permissions, longdatetime))
            {
    ...
    if (rc > 0)
    {
        ...
        WCHAR namewTmp[wdirtypemax];
        namewTmp[0] = 0;

        if (namew[0])
            wcslcpy2(namewTmp, namew, countof(namewTmp) - 1);
        else
            awlcopy(namewTmp, name, countof(namewTmp) - 1);
        ...
        else if (ConnectSettings->utf8names)
        {
            ...
            ConvertUTF8toUTF16(&srcstart, srcstart + strlen(name) + 1, &trgstart,
                               trgstart + countof(FindData->cFileName) - 1);
        }
        else
        {
        ...
        // try to extract user and group info
        CHAR buf[2];
        buf[0] = completeline[0];
        buf[1] = 0;

        CharUpper(buf);

        // a line should start like this
        if (strchr("-DLFBCP|", buf[0]))
        {
            // drwxr-xr-x  2 user       group         4096
            RemoveDoubleSpaces(completeline);

            std::vector<DWORD> v;

            // drwxr-xr-x 2 user group 4096
            SplitString(completeline, ' ', &v);

            // check if the line looks ok to be parsed
            if (v.size() >= 4 && IsNumeric(&completeline[LOWORD(v[1])], HIWORD(v[1])))
            {
                pFileInfoExtra info = new tFileInfoExtra;

                if (info)
                {
                    awlcopy(info->FileName, "/", countof(info->FileName) - 1);
                    awlcopy(&info->FileName[1], ConnectSettings->DisplayName, countof(info->FileName) - 2);
                    wcslcat(info->FileName, ConnectSettings->lastactivepath, countof(info->FileName) - 1);
                    wcslcatbackslash(info->FileName, countof(info->FileName) - 1);
                    wcslcat(info->FileName, FindData->cFileName, countof(info->FileName) - 1);

                    ReplaceBackslashBySlashW(info->FileName);

                    ZeroMemory(info->szUser, countof(info->szUser));
                    ZeroMemory(info->szGroup, countof(info->szGroup));
                    ...
                    if (file.flags & LIBSSH2_SFTP_ATTR_PERMISSIONS)
                    {
                        info->szPermissions[0] =
                            (file.permissions & LIBSSH2_SFTP_S_IFLNK) == LIBSSH2_SFTP_S_IFLNK ? 'L' : '-';
                        info->szPermissions[1] = (char)48;
                        info->szPermissions[2] = (char)48;
                        info->szPermissions[3] = (char)48;

                        // RWXRWXRWX
                        if (file.permissions & LIBSSH2_SFTP_S_IRUSR)
                            info->szPermissions[1] += 4;
                        if (file.permissions & LIBSSH2_SFTP_S_IWUSR)
                            info->szPermissions[1] += 2;
                        if (file.permissions & LIBSSH2_SFTP_S_IXUSR)
                            info->szPermissions[1] += 1;

                        if (file.permissions & LIBSSH2_SFTP_S_IRGRP)
                            info->szPermissions[2] += 4;
                        if (file.permissions & LIBSSH2_SFTP_S_IWGRP)
                            info->szPermissions[2] += 2;
                        if (file.permissions & LIBSSH2_SFTP_S_IXGRP)
                            info->szPermissions[2] += 1;

                        if (file.permissions & LIBSSH2_SFTP_S_IROTH)
                            info->szPermissions[3] += 4;
                        if (file.permissions & LIBSSH2_SFTP_S_IWOTH)
                            info->szPermissions[3] += 2;
                        if (file.permissions & LIBSSH2_SFTP_S_IXOTH)
                            info->szPermissions[3] += 1;

                        if (info->szPermissions[0] == 'L' && namewTmp[0])
                        {
                            WCHAR *p = wcsstr(namewTmp, L" -> ");

                            if (p)
                                wcslcpy2(info->wszLinkDestination, &p[4], countof(info->wszLinkDestination) - 1);
                        }
                    }
                    else
...
int SftpRenameMoveFileW(void *serverid, WCHAR *OldName, WCHAR *NewName, BOOL Move, BOOL Overwrite, BOOL isdir)
{
    ...
    if (!Overwrite)
    {
        if (ConnectSettings->scponly)
        {
            ...
            BOOL needquotes2 =
                wcschr(NewName2W, ' ') != NULL || wcschr(NewName2W, '(') != NULL || wcschr(NewName2W, ')') != NULL;
            if (needquotes2)
    ...
    else
    {
        ...
        BOOL doAgain;
        BOOL wantKeepTimestamp2 = ConnectSettings->wantKeepTimestamp;
        do
        {
            doAgain = false; // try with -p first to copy with timestamps, repeat in case of error
            wcslcpy(cmdname, Move ? L"mv " : L"cp ", countof(cmdname) - 1);
            if (wantKeepTimestamp2 && !Move)
                wcslcat(cmdname, L"-p ", countof(cmdname) - 1);

            BOOL needquotes1 =
                wcschr(OldName2W, ' ') != NULL || wcschr(OldName2W, '(') != NULL || wcschr(OldName2W, ')') != NULL;
            BOOL needquotes2 =
                wcschr(NewName2W, ' ') != NULL || wcschr(NewName2W, '(') != NULL || wcschr(NewName2W, ')') != NULL;
            if (needquotes1)
                wcslcat(cmdname, L"\"", countof(cmdname) - 1);
            ...
            if (needquotes2)
                wcslcat(cmdname, L"\"", countof(cmdname) - 1);
            char reply[1024];
            if (SftpQuoteCommand2W(serverid, NULL, cmdname, reply, sizeof(reply) - 1) == 0)
            {
                return SFTP_OK;
            }
            else if (wantKeepTimestamp2 && !Move)
            {
                // in case of error with -p, retry in any case
                // turn off -p permanently only if we can be sure that it is the reason for the error
                wantKeepTimestamp2 = false;
                char *p = strstr(reply, "cp:");
                char *p2 = NULL;
                char *p3 = NULL;
                if (p)
                {
                    ...
                    if (p3 && p3 - p >= 4)
                    {
                        p3[0] = 0;
                        p3 -= 4;
                        if (p3 > p && p3[2] == ' ' && p3[3] == 'p' || //    cp: invalid option - p
                            p3[0] == ' ' && p3[2] == 'p')
                        { // cp: invalid option -- 'p'
                            ConnectSettings->wantKeepTimestamp = false;
                            wantKeepTimestamp2 = false;
                        }
                    }
                }
...
// the doc says we should use at least 32K but internally libssh2 limits the size to 30K which leads to more packets
// than needed see discussion: https://github.com/libssh2/libssh2/discussions/1064
#define RECV_BLOCK_SIZE 30000 // 32768
#define MAX_RECV_BLOCK_SIZE 3 * 1000 * 1000

int ConvertCrToCrLf(char *data, int len, BOOL *pLastWasCr)
{
    BOOL LastWasCr = *pLastWasCr; // don't convert 0d0a->0d0d0a!
    char *data2 = (char *)malloc(sizeof(char) * len * 2);
                      ...
                      FILETIME *ft, BOOL Resume)
{
    ...
    HANDLE localfile;
    char *data = (char *)malloc(sizeof(char) * RECV_BLOCK_SIZE);
    char *dataTmp = NULL;
    ...
    if (scpdata)
    {
        ...
        {
            libssh2_session_flag(ConnectSettings->session, LIBSSH2_FLAG_QUOTE_PATHS, 0); // we quote ourselves
            if (EscapeWithDoubleQuotes(filename2, filename, sizeof(filename2) - 1) < 0)
                return SFTP_FAILED;
        }
        else
            strlcpy(filename2, filename, sizeof(filename2) - 1);
        fileinfoscp.st_size = -1;
        do
        {
        ...
        // Try again with double quotes for Hetzner storage box bug
        int lasterr = libssh2_session_last_errno(ConnectSettings->session);
        if (!SSH_ScpNeedQuote && remotefilescp == 0 && strchr(filename, ' ') != 0 &&
            (LIBSSH2_ERROR_SCP_PROTOCOL == lasterr || LIBSSH2_ERROR_CHANNEL_REQUEST_DENIED == lasterr))
        {
            if (EscapeWithDoubleQuotes(filename2, filename, sizeof(filename2) - 1) > 0)
            {
                libssh2_session_flag(ConnectSettings->session, LIBSSH2_FLAG_QUOTE_PATHS, 0); // we quote ourselves
                ...
                } while (remotefilescp == 0 &&
                         libssh2_session_last_errno(ConnectSettings->session) == LIBSSH2_ERROR_EAGAIN);
                if (remotefilescp)
                    SSH_ScpNeedQuote = true;
                else
                    libssh2_session_flag(ConnectSettings->session, LIBSSH2_FLAG_QUOTE_PATHS,
                                         1); // let the library handle it again
            }
        }

        if (!remotefilescp || fileinfoscp.st_size == -1)
        {
            if (!remotefilescp)
            ...
            else
            {
                ...
                if (rcerr > 0)
                {
                    ...
                    LogProc(PluginNumber, MSGTYPE_IMPORTANTERROR,
                            "SCP download error: No size returned for file."); // Dropbear - crashes if we don't check
                                                                               // tghis!
                // don't close, causes crash too!
                myfree(data, NULL);
                return SFTP_READFAILED;
            }
    ...
    if (Resume)
    {
        localfile = CreateFileT(LocalName, GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING,
        ...
        if (localfile != INVALID_HANDLE_VALUE)
        {
            ...
            if (filesize <= sizeloaded || sizeloaded >= ((__int64)1 << 31) - 1)
    ...
    {
        truncate = alwaysoverwrite;
        localfile = CreateFileT(
            LocalName, GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL,
            alwaysoverwrite
                ? OPEN_ALWAYS
                : CREATE_NEW, // new: only truncate once the download has started, to not lose the old file on error
            FILE_ATTRIBUTE_NORMAL | FILE_FLAG_SEQUENTIAL_SCAN, NULL);
    }
    if (localfile == INVALID_HANDLE_VALUE)
    {
    ...
    int maxblocksizeOld = maxblocksize;
    int aborttime = -1;
    ...
    do
    {
        ...
        {
            DWORD written;
            if (TextMode && sizeloaded == 0)
            { // test first block if it's binary
                for (int i = 0; i < len; i++)
                    if (data[i] == 0)
                    {
                        TextMode = false;
        ...
        if (current - start >= 1000 && lenSum >= maxblocksize)
        {
            // increase buffer size
            maxblocksizeOld = maxblocksize;
            maxblocksize = maxblocksize * RECV_BLOCK_SIZE / (current - start);

            if (maxblocksize > RECV_BLOCK_SIZE && maxblocksize % RECV_BLOCK_SIZE != 0)
                maxblocksize = RECV_BLOCK_SIZE * ((maxblocksize + RECV_BLOCK_SIZE - 1) / RECV_BLOCK_SIZE);

            if (maxblocksize > MAX_RECV_BLOCK_SIZE)
                maxblocksize = MAX_RECV_BLOCK_SIZE;
            else if (maxblocksize < RECV_BLOCK_SIZE)
                maxblocksize = RECV_BLOCK_SIZE;

            if (maxblocksize != maxblocksizeOld && maxblocksizeOld < maxblocksize)
            {
                dataTmp = data;

                data = (char *)realloc(data, sizeof(char) * maxblocksize);

                if (data == NULL)
                {
                    data = dataTmp;

                    maxblocksize = maxblocksizeOld;
                }
                else if (TextMode)
                    maxblocksize /= 2; // in worst case, we have all line breaks (0A)
    ...
    if (retval != SFTP_ABORT && retval != SFTP_WRITEFAILED && sizeloaded < filesize && sizeloaded > resumepos &&
        !ConnectSettings->scponly)
        retval = SFTP_PARTIAL;
...
// the doc says we should use at least 32K but internally libssh2 limits the size to 30K which leads to more packets
// than needed see discussion: https://github.com/libssh2/libssh2/discussions/1064
#define SEND_BLOCK_SIZE 30000 // 16384
#define MAX_SEND_BLOCK_SIZE 3 * 1000 * 1000
...
int SftpUploadFileW(void *serverid, WCHAR *LocalName, WCHAR *RemoteName, BOOL Resume, BOOL setattr)
{
    ...
    char *data = (char *)malloc(sizeof(char) * SEND_BLOCK_SIZE);
    char *dataTmp = NULL;
    ...
    localfile = CreateFileT(LocalName, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING,
                            FILE_ATTRIBUTE_NORMAL | FILE_FLAG_SEQUENTIAL_SCAN, NULL);
    ...
    if (localfile != INVALID_HANDLE_VALUE)
    {
        ...
        if (scpdata && filesize > (((__int64)1) << 31)) // scp supports max 2 GB
        {
            ...
            // But SCP on server side needs to be 64bit
            if (!SSH_ScpNo2GBLimit ||
                (ConnectSettings->scpserver64bit != 1 && !ConnectSettings->scpserver64bittemporary))
            {
                if (!SSH_ScpNo2GBLimit)
                {
                    if (ConnectSettings->scponly)
        ...
        if (scpdata)
        {
            ...
            {
                libssh2_session_flag(ConnectSettings->session, LIBSSH2_FLAG_QUOTE_PATHS, 0); // we quote ourselves
                if (EscapeWithDoubleQuotes(thename2, thename, sizeof(thename2) - 1) < 0)
                    return SFTP_FAILED;
            }
            else
            ...
            {
                if (!SSH_ScpNo2GBLimit)
                    remotefilescp = libssh2_scp_send_ex(ConnectSettings->session, thename2, ConnectSettings->filemod,
                                                        (int)filesize, mtime, 0);
                else
                    remotefilescp = libssh2_scp_send64(ConnectSettings->session, thename2, ConnectSettings->filemod,
                                                       (libssh2_uint64_t)filesize, mtime, 0);

                if (UpdatePercentBar(serverid, 0))
                {
                    ConnectSettings->neednewchannel = true;
                    break;
                }
            ...
            // Try again with double quotes for Hetzner storage box bug
            int lasterr = libssh2_session_last_errno(ConnectSettings->session);
            if (!SSH_ScpNeedQuote && remotefilescp == 0 && strchr(thename, ' ') != 0 &&
                (LIBSSH2_ERROR_SCP_PROTOCOL == lasterr || LIBSSH2_ERROR_CHANNEL_REQUEST_DENIED == lasterr))
            {
                if (EscapeWithDoubleQuotes(thename2, thename, sizeof(thename2) - 1) > 0)
                {
                    ...
                    {
                        if (!SSH_ScpNo2GBLimit)
                            remotefilescp = libssh2_scp_send_ex(ConnectSettings->session, thename2,
                                                                ConnectSettings->filemod, (int)filesize, mtime, 0);
                        else
                            remotefilescp =
                                libssh2_scp_send64(ConnectSettings->session, thename2, ConnectSettings->filemod,
                                                   (libssh2_uint64_t)filesize, mtime, 0);
            ...
            if (rcerr > 0)
            {
                ...
                if (strstr(errbuf, "scp: "))
                {
                    LogProc(PluginNumber, MSGTYPE_IMPORTANTERROR, errbuf);
                    CloseHandle(localfile);
                    // CloseRemote(serverid,0,remotefilescp,false,0); <- do not call, causes crash later!
                    myfree(data, NULL);
                    return SFTP_WRITEFAILED;
                }
            }
        ...
        else
        {
            ...
            do
            {
                remotefilesftp = libssh2_sftp_open(ConnectSettings->sftpsession, thename,
        ...
        if (remotefilescp || remotefilesftp)
        {
            ...
            DWORD starttime;
            DWORD len;
            __int64 writtenSum = 0;
            DWORD start = GetTickCount();
            DWORD current = 0;
            retval = SFTP_OK;

            int maxblocksize = SEND_BLOCK_SIZE;
            int maxblocksizeOld = maxblocksize;

            while (ReadFile(localfile, data, maxblocksize, &len, NULL) && len > 0)
            {
                ...
                do
                {
                    ...
                    {
                        if (written > (int)len)
                        { // libssh2_channel_write sometiomes returns values > len!
                            retval = SFTP_WRITEFAILED;
                ...
                if (current - start >= 1000 && writtenSum >= maxblocksize)
                {
                    // increase buffer size
                    maxblocksizeOld = maxblocksize;
                    maxblocksize = maxblocksize * SEND_BLOCK_SIZE / (current - start);

                    if (maxblocksize > SEND_BLOCK_SIZE && maxblocksize % SEND_BLOCK_SIZE != 0)
                        maxblocksize = SEND_BLOCK_SIZE * ((maxblocksize + SEND_BLOCK_SIZE - 1) / SEND_BLOCK_SIZE);

                    if (maxblocksize > MAX_SEND_BLOCK_SIZE)
                        maxblocksize = MAX_SEND_BLOCK_SIZE;
                    else if (maxblocksize < SEND_BLOCK_SIZE)
                        maxblocksize = SEND_BLOCK_SIZE;
...
BOOL SftpChmodW(void *serverid, WCHAR *RemoteName, WCHAR *chmod)
{
    ...
    if (ConnectSettings->scponly || (chmod[3] >= '0' && chmod[3] <= '9'))
    {
        ...
        BOOL needquotes =
            wcschr(RemoteName, ' ') != NULL || wcschr(RemoteName, '(') != NULL || wcschr(RemoteName, ')') != NULL;
        if (needquotes)
...
BOOL SftpLinkFolderTargetW(void *serverid, WCHAR *RemoteName, int maxlen)
{
    ...
    if (strcmp(filename, "/~") == 0 || strcmp(filename, "/home/~") == 0)
    { // go to home dir special link
        char ReturnedName[wdirtypemax];
        WCHAR cmdname[MAX_PATH];
        wcslcpy(cmdname, L"echo $HOME", countof(cmdname) - 1);
        ReturnedName[0] = 0;
        if (SftpQuoteCommand2W(ConnectSettings, NULL, cmdname, ReturnedName, wdirtypemax - 1) == 0 &&
            ReturnedName[0] == '/')
        {
    ...
    else
    {
        ...
        { // follow link without SFTP functions
        ...
        if (linktarget[0])
        {
            ...
            if (ConnectSettings->utf8names)
            {
                ...
                ConvertUTF8toUTF16(&srcstart, (UTF8 *)linktarget + strlen(linktarget) + 1, &trgstart,
                                   trgstart + wdirtypemax - 1);
            }
            else
                     ...
                     int errbuflen)
{
    ...
    do
    {
        ...
        else if (rc == LIBSSH2_ERROR_EAGAIN)
        {
            ...
            if (thisdatatime - lastdatatime < 1000 || thisdatatime - startdatatime < 5000)
                rc = 1;
        }
        if (endreceived && rc <= 0 && rc != LIBSSH2_ERROR_EAGAIN)
        {
            ...
            {
                if (detectingcrlf)
                { // only append it once - do not use this appended to detect!
                    detectingcrlf = false;
                    strlcat(msgbuf, "\r\n", sizeof(msgbuf) - 1);
                }
            }
...
{
    WCHAR transferMode = mode[0];
    CharUpperBuffW(&transferMode, 1);
    Global_TransferMode = (char)transferMode;
...
myint __stdcall PropDlgProc(HWND hWnd, unsigned int Message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG: {
        ...
        BOOL hasField;
        HDC dc = GetDC(hWnd);
        HFONT fixedfont = CreateFont(-MulDiv(8, GetDeviceCaps(dc, LOGPIXELSY), 72), 0, 0, 0, FW_NORMAL, 0, 0, 0,
        ...
        if (!g_command_ls)
        {
            if (usys())
            {
                ...
                if (!wp)
                    wp = FindStatStringW(g_statreplyW, L"Datei:", &chw);
                if (wp)
                {
            ...
            else
            {
                ...
                if (!p)
                    p = FindStatString(g_statreplyA, "Datei:", &ch);
                if (p)
                {
            ...
            if (!p)
                p = FindStatString(g_statreplyA, "Zugriff:", &ch);
            if (p)
            {
            ...
            {
                p = strstr(g_statreplyA, "Uid:"); // same line as Uid, in front of it
                if (!p)
                    p = strstr(g_statreplyA, "UID:");
                if (p)
                {
                    p2 = p;
                    while (p2 > g_statreplyA && p2[0] != ':' && p2[0] != '\n')
                        p2--;
                    if (p2 > g_statreplyA && p2 < p)
                    {
                        p2++;
                        while (p2[0] == ' ')
                            p2++;
                        p[0] = 0;
                        SetDlgItemText(hWnd, IDC_PROP_PERMISSIONS, p2);
                        p[0] = 'U';
                    }
                }
            }
            ...
            if (!p)
                p = FindStatString(g_statreplyA, "Modifiziert:", &ch);
            if (!p)
            {
                p = strstr(g_statreplyA, "Uid:"); // 2 lines below Uid, must have correct format
                if (!p)
                    p = strstr(g_statreplyA, "UID:");
                if (p)
                    p = strchr(p, '\n');
                ...
                if (p)
                {
                    ...
                    if (p)
                    {
                        while (p[0] == ' ')
                            p++;
                        if (!(strlen(p) > 20 && p[4] == '-' && p[7] == '-' && p[10] == ' ' && p[13] == ':' &&
                              p[16] == ':' && p[19] == '.'))
                            p = NULL;
                    }
                }
...
void SftpShowPropertiesW(void *serverid, WCHAR *remotename)
{
    ...
    if (statworked)
    {
        ...
        if (!wp)
            wp = FindStatStringW(g_statreplyW, L"Datei:", &chw);
        if (!wp)
            wp = FindStatStringW(g_statreplyW, L"Uid:", &chw); // other languages
        if (!wp)
            wp = FindStatStringW(g_statreplyW, L"UID:", &chw); // other languages
        if (wp)
        {
    ...
    if (!statworked)
    { // stat failed -> try "ls -la filename"
...
int SftpGetFileChecksumResultW(BOOL WantResult, HANDLE ChecksumHandle, void *serverid, char *checksum, int maxlen)
{
    ...
    else
    {
        ...
        {
            buf[0] = 3;
            while (libssh2_channel_write_ex(channel, 0, buf, 1) == LIBSSH2_ERROR_EAGAIN)
            { // Ctrl+C!
                if (EscapePressed())
                    break;
            }
...
{
    char buf[32768];
    OverwriteWithZeroes(buf, sizeof(buf));
}
