...
int LoadServersFromIni(char *inifilename, char *quickconnectname)
{
    ...
    { // list exists -> update!
        ...
        while (thisentry)
        {
            if (!thisentry->serverupdated && thisentry->serverid == NULL &&
                (quickconnectname == NULL || strcmp(thisentry->displayname, quickconnectname) != 0))
            {
                if (preventry)
                    preventry->next = NULL;
    ...
    else
    {
        ...
        __try
        {
            if (background_linked_list)
            {
                ...
                while (thisentry)
                {
                    if (_stricmp(thisentry->displayname, displayname) == 0 && thisentry->threadid == id)
                    {
                        if (thisentry->serverid)
                            free(thisentry->serverid);
                        if (newid)
