...
{
    if (ConnectSettings && info)
    {
        if (gFileInfoExtras.size() == 0)
            gFileInfoExtras.reserve(20000);

        gFileInfoExtras.push_back(info);
    }
}
...
void ResetFileInfoExtras(void *serverid)
{
    if (serverid == NULL)
    {
        ...
        for (unsigned int i = 0; i < gFileInfoExtras.size(); i++)
        {
    ...
    if (ConnectSettings)
    {
        ...
        awlcopy(&wTmp[1], ConnectSettings->DisplayName, countof(wTmp) - 2);
        wcslcat(wTmp, ConnectSettings->lastactivepath, countof(wTmp) - 1);
        wcslcatbackslash(wTmp, countof(wTmp) - 1);
        ...
        while (i < gFileInfoExtras.size())
        {
            ...
            if (pTmp && wcsstr(pTmp->FileName, wTmp))
            {
                delete pTmp;
...
pFileInfoExtra FindFileInfoExtras(WCHAR *RemoteName)
{
    ...
    WCHAR wTmp[2048];
    wcslcpy2(wTmp, RemoteName, countof(wTmp) - 1);
    ...
    for (size_t i = 0; i < gFileInfoExtras.size(); i++)
    {
        ...
        if (pTmp && _wcsicmp(pTmp->FileName, wTmp) == 0)
            return pTmp;
    }
...
HANDLE __stdcall FsFindFirstW(WCHAR *Path, WIN32_FIND_DATAW *FindData)
{
    ...
    else
    {
        ...
        BOOL ok = SFTP_OK == SftpFindFirstFileW(serverid, remotedir, &sftpdataptr, &skippedchar);

        if (wcslen(remotedir) <= 1 || wcscmp(remotedir + 1, L"home") == 0)
        { // root -> add ~ link to home dir
            SYSTEMTIME st;
            FindData->dwFileAttributes = 0;
...
BOOL __stdcall FsFindNextW(HANDLE Hdl, WIN32_FIND_DATAW *FindData)
{
    ...
    if (lf != INVALID_HANDLE_VALUE)
    {
        ...
        else if (lf->rootfindhandle)
        {
            ...
            return SftpFindNextFileW(lf->serverid, lf->sftpdataptr, FindData, 0) == SFTP_OK;
    }
    return false;
}
...
int __stdcall FsExecuteFileW(HWND MainWin, WCHAR *RemoteName, WCHAR *Verb)
{
    ...
    else if (_wcsnicmp(Verb, L"mode ", 5) == 0)
    { // Binary/Text/Auto
...
int __stdcall FsGetFileW(WCHAR *RemoteName, WCHAR *LocalName, int CopyFlags, RemoteInfoStruct *ri)
{
    ...
    if (OverWrite)
    {
        // DeleteFileT(LocalName);  // don't delete it! We can keep it when the download fails to start!
    }
    else
    {
    ...
    { // auto-resume loop
        ...
        case SFTP_EXISTS:
            return FS_FILE_EXISTS;
        case SFTP_READFAILED:
            return FS_FILE_READERROR;
        case SFTP_WRITEFAILED:
            return FS_FILE_WRITEERROR;
        case SFTP_ABORT:
            return FS_FILE_USERABORT;
        case SFTP_PARTIAL:
...
int __stdcall FsPutFileW(WCHAR *LocalName, WCHAR *RemoteName, int CopyFlags)
{
    ...
    case SFTP_OK:
        return FS_FILE_OK;
    case SFTP_EXISTS:
        return SftpSupportsResume(serverid) ? FS_FILE_EXISTSRESUMEALLOWED : FS_FILE_EXISTS;
    case SFTP_READFAILED:
        return FS_FILE_READERROR;
    case SFTP_WRITEFAILED:
        return FS_FILE_WRITEERROR;
...
#define fieldcount 4
int fieldtypes[fieldcount] = {ft_string, ft_string, ft_string, ft_string};
                                 ...
                                 int maxlen, int flags)
{
    ...
    pFileInfoExtra pInfoS = FindFileInfoExtras(FileName);
